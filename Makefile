...
#
#
# rules:  $XFree86: xc/config/cf/Imake.rules,v 3.94 2001/04/26 21:27:48 dawes Exp $
...
# - for ESD support -
# AUDIODEVICE =
# AUDIODEFINES = -DESDAUDIO
# AUDIOLIB = -L/usr/local/lib -lesddsp -lesd
...
DEFINES = -DXLVNSPACKDIR=\"$(XLVNSPACKDIR)\" -DCDDEVICE=\"$(CDDEVICE)\" -DAUDIODEVICE=\"$(AUDIODEVICE)\" $(AUDIODEFINES) -DMITSHM
...
install:: xlvns
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  xlvns $(DESTDIR)$(BINDIR)/xlvns

all:: xlvns.$(MANNEWSUFFIX)

xlvns.$(MANNEWSUFFIX): xlvns.$(MANSRCSUFFIX)
	$(RM) $@
	$(LN) xlvns.$(MANSRCSUFFIX) $@

cleandir::
	$(RM) xlvns.$(MANNEWSUFFIX)

install.man:: xlvns.$(MANNEWSUFFIX)
	@if [ -d $(DESTDIR)$(MANDIR) ]; then set +x; \
	else (set -x; $(MKDIRHIER) $(DESTDIR)$(MANDIR)); fi
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) xlvns.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/xlvns.$(MANSUFFIX)
...
install:: XLvns.ad
	...
	@if [ -f $(DESTDIR)$(XAPPLOADDIR)/XLvns ]; then \
	echo "Not overwriting existing" $(DESTDIR)$(XAPPLOADDIR)/XLvns; \
	else set -x; \
	$(INSTALL) $(INSTALLFLAGS) $(INSTAPPFLAGS) XLvns.ad $(DESTDIR)$(XAPPLOADDIR)/XLvns; fi

install:: sizfont.tbl
	$(INSTALL) $(INSTALLFLAGS) $(INSTDATFLAGS) sizfont.tbl $(DESTDIR) $(XLVNSPACKDIR)

install:: kizfont.tbl
	$(INSTALL) $(INSTALLFLAGS) $(INSTDATFLAGS) kizfont.tbl $(DESTDIR) $(XLVNSPACKDIR)

install:: thfont.tbl
	$(INSTALL) $(INSTALLFLAGS) $(INSTDATFLAGS) thfont.tbl $(DESTDIR) $(XLVNSPACKDIR)
